...
#    X_test = scaler.transform(X_test)
    ...
    net, final_loss, learning_curve = train_neural_net(model, loss_fn, X=X_train, y=y_train, n_replicates=3, max_iter=max_iter)
    ...
    e = (y_test_est != y_test)
    error_rate = (torch.sum(e).type(torch.float)/len(y_test)).data.numpy()
